...
struct FloatAligner{
    ...
    void initial_calc32_local_affine_float(const int value, char query_letter, float& E, float& penalty_here31, float penalty_diag, float penalty_left, float& maximum, 
        ...
        const float* const sbt_row = &shared_matrix[int(query_letter) * deviceSubMatrixDim];
...
void NW_local_affine_single_pass_float(
    ...
    using Processor = FloatAligner<numRegs, subMatrixDim, PositionsIterator>;

    //__shared__ typename Processor::BLOSUM62_SMEM shared_matrix;

    //25 is max substitution matrix dimension
    __shared__ float shared_matrix[25 * 25];
...
void launch_process_overflow_alignments_kernel_NW_local_affine_multi_pass_float(
    ...
    if(numOverflow > 0){
        ...
        for(size_t iter = 0; iter < numIters; iter++){
            ...
            NW_local_affine_multi_pass_float<numRegs, subMatrixDim><<<num, 32>>>(
                devChars, 
                devAlignmentScores,
                d_tempHcol2, 
                d_tempEcol2, 
